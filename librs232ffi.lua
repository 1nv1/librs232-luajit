...
local Lib = ffi.load ("./librs232.so.1.0")
...
local port = ffi.new ("char[?]", 10, "/dev/ttyS0")
local datos = ffi.new ("char[?]", lenBuffer) -- Buffer para los datos
...
if fd < 0 then
	print ("Error, no se puede abrir el puerto.")
	os.exit()
end
...
Lib.serial_close(fd) -- Cierro el puerto
